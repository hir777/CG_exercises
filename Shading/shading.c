...
// The function which calculates the inner product of given two vectors
...
static void PhongModel(GLfloat* pe, GLfloat* ne, GLfloat* out_color) {
  ...
  for( i = 0; i < g_num_lights; i++ ) {
    ...
    for( j = 0; j < 4; j++ )
      h[j] = v[j] + L[j];
    for( j = 0; j < 4; j++ )
      h[j] /= sqrt( innerProduct(h, h, 4) );

    // Calculate Is = ks * I * max(h・n, 0)^m
    tmp = pow( fmaxf( innerProduct(h, ne, 4), 0.0), 4.0);
